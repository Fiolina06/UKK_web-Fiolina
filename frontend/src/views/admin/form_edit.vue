<script> 
    ...
    export default{
        ...
        methods:{
            async getBukuById(){
                try{
                    const ress =  await axios.get(`books/${this.idParams}`)
                    this.book.id = `${this.idParams}`
                    this.book.judul = ress.data.judul
                    this.book.pengarang = ress.data.pengarang
                    this.book.penerbit = ress.data.penerbit
                    this.book.tahun_terbit = ress.data.tahun_terbit
                    this.imageUrl = `http://127.0.0.1:8000/storage/${ress.data.gambar}`
                }
                catch(err){
                    console.log(err)
                }
            ...
            editBook() {
                ...
                axios.post(`books/${this.idParams}`, this.formData, {
                    headers: {
...
<template>
    <div class="flex flex-col items-center">
      ...
      <form class="w-full max-w-lg" @submit.prevent="editBook">
        <div class="mb-4">
